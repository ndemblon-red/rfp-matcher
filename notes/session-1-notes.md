...
An internal Flask web app that syncs a consulting firm's case studies from a live PPTX file into a searchable SQLite database, with a library view and the foundations of an RFP matching engine. The app reads directly from a OneDrive-synced SharePoint folder so data is always live.
...
**What happened:** The project started with a plan to use an Excel inventory file as the metadata source, matching it to PPTX slides for content. This seemed sensible — the Excel already had industry, AI type, and slide numbers all neatly organised.

**The problems:** Slide number mismatches, duplicate project titles across different clients, stale flags from a previous exercise, client names that didn't match between files, fuzzy matching failures at scale.
...
**What happened:** Previous projects were built in Claude.ai chat sessions. This project switched to Claude Code.
...
**The requirement:** The app needed to always read the live PPTX from SharePoint, not stale local copies. This seemed straightforward but turned into one of the most friction-heavy parts of the build.
...
**Attempt 3 — OneDrive Sync:** Settled on syncing the SharePoint folder to local machine via OneDrive. Simple, no admin required, files always live. This worked — but introduced a new problem.
...
**Attempt 4 — `attrib` command to trigger recall:** Unreliable — sometimes worked, sometimes didn't.

**Attempt 5 — Retry loop with PermissionError handling:** Helped with locking but didn't solve the cloud-only placeholder problem.
...
- **Lines of code in app.py:** ~170 (well under blueprint threshold at time of writing)
- **Tests passing:** 32+
